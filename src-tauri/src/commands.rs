...
pub fn capture_screen() -> Result<String, String> {
    ...
    #[cfg(target_os = "macos")]
    {
        // Use macOS native screencapture — handles Screen Recording permissions better
        let tmp_path = "/tmp/copas_screenshot.png";
        ...
        if let Ok(result) = output {
            ...
            log::warn!("screencapture exit code: {}", result.status);
        }
        // Fall through to xcap fallback
    }

    // Fallback / Windows: use xcap
    {
        use xcap::Monitor;
        use std::io::Cursor;
